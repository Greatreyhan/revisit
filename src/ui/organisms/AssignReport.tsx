...
interface AssignReportProps {
    reportID: string[];
    setReportID: (nextReportIDs: string[]) => void;
}

const AssignReport: React.FC<AssignReportProps> = ({ reportID, setReportID }) => {
    const { getFromDatabase, user } = useFirebase();
    const [dataReport, setDataReport] = useState<{ [key: string]: ReportData }>({});
    ...
    const handleRemove = (id: string) => {
        setReportID(reportID.filter((rid) => rid !== id));
    };
        ...
        <>
            <div className="w-full py-8 px-8 rounded-lg my-4 bg-slate-100">
                <div className="flex justify-between">
                    <h2 className="font-semibold mb-4">Investigation Report</h2>
                    <button type="button"
                        onClick={openModal}
                            ...
                            {reportID.map((key, i) => (
                                <tr key={key} className="text-gray-700 md:text-md text-sm">
                                    ...
                                    <td className="border p-4">
                                        {[
                                            dataReport[key]?.largeClassification,
                                            dataReport[key]?.middleClassification,
                                            dataReport[key]?.partProblem,
                                        ]
                                            .filter(Boolean)
                                            .join(" ")}
                                    </td>
                                    <td className="border p-4 md:table-cell hidden">
                                        {[dataReport[key]?.focusModel, dataReport[key]?.euroType]
                                            .filter(Boolean)
                                            .join(" ")}
                                    </td>
                                    <td className="border p-4">{dataReport[key]?.visitDate}</td>
                                ...
                                <label key={key} className="flex items-center gap-2">
                                    ...
                                    <span>
                                        {[
                                            ...
                                            .filter(Boolean)
                                            .join(" ")} — {dataReport[key]?.customerName}
                                    </span>
                                </label>
                            ))}
