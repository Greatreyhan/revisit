...
const ProfileHealthEditor: React.FC = () => {
    ...
    const handleSendData = async (e: React.FormEvent) => {
        ...
        try {
            await saveToDatabase(`health/${user?.uid}/${id || Date.now()}`, newData);
            ...
            <div className="pt-16">
                ...
                <form className="md:w-10/12 w-11/12 flex flex-col mx-auto my-4 justify-around items-center" onSubmit={handleSendData}>
                    ...
                    <div className="w-full py-8 px-8 rounded-lg my-4 bg-slate-100">
                        ...
                        <div className="md:flex w-full gap-5">
                            <InputField label="Download Date" type="datetime-local" name="dateStart" value={downloadDate} onChange={handleChange(setDownloadDate)} placeholder="Download Date" />
                            <InputField label="Explain Date" type="datetime-local" name="dateEnd" value={explainDate} onChange={handleChange(setExplainDate)} placeholder="Explain Date" />
                        </div>
                        <div className="md:flex w-full gap-5">
                            <SelectInput label="Tipe Unit" name="typeUnit" value={typeUnit} onChange={(e) => setTypeUnit(e.target.value)} options={dataModelIsuzu} />
                        ...
                        <div className="md:flex w-full gap-5">
                            <SelectInput
                                ...
                                options={segmentData}
                            />
                            <InputField type="number" label="Konsumsi Bahan Bakar (km/l)" name="fuelConsumption" value={fuelConsumption} onChange={(e) => setFuelConsumption(e.target.value)} placeholder="Fuel Consumption" />
                        </div>
                        {/* Attachment */}
