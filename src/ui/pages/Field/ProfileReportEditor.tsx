...
const ProfileReportEditor: React.FC = () => {
    ...
    const handleSendData = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await saveToDatabase(`report/${user?.uid}/${id || dateNow}`, newData);
            // Add data to customer
            if (customerId) {
                const dataReportID = getReportID(customerId)
                if (id) {
                    // check if id exist
                    if (!dataReportID.includes(id)) {
                        dataReportID.push(id)
                        await saveToDatabase(`customer/${user?.uid}/${customerId}/reportID`, dataReportID);
                    }
                }
                else {
                    dataReportID.push(dateNow.toString())
                    await saveToDatabase(`customer/${user?.uid}/${customerId}/reportID`, dataReportID);
                }
            }
            waiting(false);
