...
const ProfileTrainingEditor: React.FC = () => {
    ...
    const handleSendData = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await saveToDatabase(`training/${user?.uid}/${id || dateNow}`, newData);
            if (customerId) {
                const dataTrainingID = getTrainingID(customerId)
                if (id) {
                    // check if id exist
                    if (!dataTrainingID.includes(id)) {
                        dataTrainingID.push(id)
                        await saveToDatabase(`customer/${user?.uid}/${customerId}/trainingID`, dataTrainingID);
                    }
                }
                else {
                    dataTrainingID.push(dateNow.toString())
                    await saveToDatabase(`customer/${user?.uid}/${customerId}/trainingID`, dataTrainingID);
                }
            }
