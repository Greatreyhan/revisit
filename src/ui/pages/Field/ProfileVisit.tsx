...
const ProfileVisit = () => {
  ...
  const [data, setData] = useState<{ [key: string]: VisitData }>({});
  ...
  return (
    <div className="w-10/12 mx-auto pt-8">
      ...
      <div className="flex items-center justify-between py-8">
        <p className="px-4 py-2 rounded-md bg-slate-100"><span className="text-sm pr-2">Total Visit :</span> <span className="font-bold text-primary-dark">{key.length}</span></p>
        <Link
          ...
          <tbody>
            {key.map((key, i) => (
              <tr key={key} className="text-gray-700 md:text-md text-sm">
                ...
                <td className="border p-4">{data[key]?.area}</td>
