...
// Helper: array nama bulan (Bahasa Indonesia)
const namaBulan = (monthIndex: number): string => {
  const bulan = [
...
const DealerPodium: React.FC = () => {
  ...
  // State untuk cabang UIDs (UID cabang-cabang yang di‐assign ke admin)
  ...
  // Ambil cabang UIDs (cabang yang di‐assign ke user saat login)
  useEffect(() => {
    if (!user?.uid) return;
  ...
  // Filter user field berdasarkan cabang yang di‐assign
  const filteredUserKeys = useMemo(
    () =>
      userKeys.filter(
        (uid) =>
          userData[uid]?.type === "Field" && cabangUIDs.includes(uid)
      ),
  ...
  // Bangun summaryData full (tanpa filter tanggal)
  ...
  // --------------------------------------------------------
  // Fungsi Export ke Excel (semua event dipisah per baris),
  // tetapi untuk kolom “Cabang” kita ambil dari userData[uid].
  // --------------------------------------------------------
  const handleExportReport = () => {
    // Array penampung seluruh baris eksport
    const exportRows: Array<Record<string, any>> = [];

    // Helper untuk mem‐format tanggal dari ms timestamp ke "dd/mm/yyyy"
    const formatDate = (msTimestamp: number): string => {
    ...
    // Helper untuk mengambil nama bulan (dalam Bahasa Indonesia) dari ms timestamp
    const formatMonth = (msTimestamp: number): string => {
      const d = new Date(msTimestamp);
      return namaBulan(d.getMonth());
    };

    // Loop setiap user (uid) di dalam filteredSummaryData
    Object.keys(filteredSummaryData).forEach((uid) => {
      const userName = filteredSummaryData[uid].name;

      // Ambil “Cabang” dari data user; misal di userData[uid].cabang atau userData[uid].locationCabang
      // Sesuaikan nama field berikut sesuai yang ada di struktur Firebase Anda:
      const userCabang =
        userData[uid]?.cabang ||
        userData[uid]?.locationCabang ||
        userData[uid]?.lokasiCabang ||
        "-";

      // 1) Proses semua Visit (REGULAR VISIT)
      filteredSummaryData[uid].visit.forEach((id) => {
        const ts = parseInt(id);
        const eventObj = visitData[uid][id] || {};
        console.log(eventObj)
        exportRows.push({
          Bulan: formatMonth(ts),
          "User (FA)": userName,
          Cabang: eventObj.dealer || "-",
          "Tgl Kegiatan": formatDate(ts),
          "Tempat Keg": eventObj.city || "-",
          "Customer Name": eventObj.customerName || "-",
          Kegiatan: "REGULAR VISIT",
        });
      });

      // 2) Proses semua Report (INVESTIGASI)
      filteredSummaryData[uid].report.forEach((id) => {
        const ts = parseInt(id);
        const eventObj = reportData[uid][id] || {};
        exportRows.push({
          Bulan: formatMonth(ts),
          "User (FA)": userName,
          Cabang: userCabang,
          "Tgl Kegiatan": formatDate(ts),
          "Tempat Keg": eventObj.tempatKegiatan || "-",
          "Customer Name": eventObj.customerName || "-",
          Kegiatan: "INVESTIGASI",
        });
      });

      // 3) Proses semua Health (PROBLEM SOLVING)
      filteredSummaryData[uid].health.forEach((id) => {
        const ts = parseInt(id);
        const eventObj = healthData[uid][id] || {};
        exportRows.push({
          Bulan: formatMonth(ts),
          "User (FA)": userName,
          Cabang: userCabang,
      ...
      // 4) Proses semua Training (TRAINING)
      filteredSummaryData[uid].training.forEach((id) => {
        const ts = parseInt(id);
        const eventObj = trainingData[uid][id] || {};
        exportRows.push({
          Bulan: formatMonth(ts),
          "User (FA)": userName,
          Cabang: userCabang,
    ...
    // Buat worksheet dan workbook, lalu simpan
    ...
    XLSX.utils.book_append_sheet(wb, ws, "ExportKegiatan");
    XLSX.writeFile(wb, "export_kegiatan.xlsx");
  };
  // --------------------------------------------------------
        ...
        >
          ...
          >
            ...
            <h3 className="text-xl font-semibold mb-4">
              {filteredSummaryData[keyData].name}
            </h3>
            <div className="space-y-3">
              <div>
                <p className="mt-4 font-semibold">
                  {filteredSummaryData[keyData].report.length} Investigasi
                </p>
                <div className="flex flex-col gap-2 mt-2">
                  {filteredSummaryData[keyData].report.map((id) => (
                    ...
                    >
                      ...
                      <span className="text-sm w-full ml-8">
                        {reportData[keyData][id]?.customerName}
                      </span>
                      <Link
                        target="_blank"
                        to={`/dealer/report/${keyData}/${id}`}
                      >
                        <MdArrowOutward className="text-xl bg-primary rounded-full p-1 text-white cursor-pointer" />
                      </Link>
                    </div>
                  ))}
                </div>
              ...
              <div>
                <p className="mt-4 font-semibold">
                  {filteredSummaryData[keyData].visit.length} Reguler Visit
                </p>
                <div className="flex flex-col gap-2 mt-2">
                  {filteredSummaryData[keyData].visit.map((id) => (
                    ...
                    >
                      ...
                      <span className="text-sm w-full ml-8">
                        {visitData[keyData][id]?.customerName}
                      </span>
                      <Link
                        target="_blank"
                        to={`/dealer/visit/${keyData}/${id}`}
                      >
                        <MdArrowOutward className="text-xl bg-primary rounded-full p-1 text-white cursor-pointer" />
                      </Link>
                    </div>
              ...
              <div>
                <p className="mt-4 font-semibold">
                  {filteredSummaryData[keyData].health.length} Health Report
                </p>
                <div className="flex flex-col gap-2 mt-2">
                  {filteredSummaryData[keyData].health.map((id) => (
                    ...
                    >
                      ...
                      <span className="text-sm w-full ml-8">
                        {healthData[keyData][id]?.customerName}
                      </span>
                      <Link
                        target="_blank"
                        to={`/dealer/health/${keyData}/${id}`}
                      >
                        <MdArrowOutward className="text-xl bg-primary rounded-full p-1 text-white cursor-pointer" />
                      </Link>
                    </div>
              ...
              <div>
                <p className="mt-4 font-semibold">
                  {filteredSummaryData[keyData].training.length} Training Report
                </p>
                <div className="flex flex-col gap-2 mt-2">
                  {filteredSummaryData[keyData].training.map((id) => (
                    ...
                    >
                      ...
                      <span className="text-sm w-full ml-8">
                        {trainingData[keyData][id]?.customerName}
                      </span>
                      <Link
                        target="_blank"
                        to={`/dealer/training/${keyData}/${id}`}
                      >
                        <MdArrowOutward className="text-xl bg-primary rounded-full p-1 text-white cursor-pointer" />
                      </Link>
                    </div>
            ...
            {sortedKeys().map((uid, i) => (
              <tr key={uid} className="text-gray-700 text-sm md:text-md">
                ...
                <td className="border p-4">
                  {filteredSummaryData[uid].report.length}
                </td>
                <td className="border p-4">
                  {filteredSummaryData[uid].visit.length}
                </td>
                <td className="border p-4">
                  {filteredSummaryData[uid].health.length}
                </td>
                <td className="border p-4">
                  {filteredSummaryData[uid].training.length}
                </td>
                <td className="border p-4">
                  {filteredSummaryData[uid].score}
                </td>
